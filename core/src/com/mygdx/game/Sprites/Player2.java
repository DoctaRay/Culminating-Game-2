...
public class Player2 extends Sprite {
    ...
    public Player2(PlayScreen screen, MyGdxGame game) {
    ...
    public TextureRegion getFrame(float dt) {
        ...
        if
                ((b2body.getLinearVelocity().x < 0 || !runningRight) && !region.isFlipX()){
    ...
    public State getState() {
        if (isDead) {
            return Player2.State.DEAD;
        } else if(b2body.getLinearVelocity().y > 2) {
            return Player2.State.JUMPING;
        } else {
            return Player2.State.STANDING;
        }
    }
    ...
    public void definePlayer2(){
        ...
        bdef.position.set(450 / MyGdxGame.PPM, 128 / MyGdxGame.PPM);
        ...
        fdef.filter.categoryBits = MyGdxGame.PLAYER2_BIT;
        fdef.filter.maskBits = MyGdxGame.GROUND_BIT |
                MyGdxGame.PLAYER1_BIT|
                MyGdxGame.LOSE_BIT|
                MyGdxGame.SPEED_BIT;
